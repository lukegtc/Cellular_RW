...
from src.models.gin import GINLSPE
...
class LitGINModel(pl.LightningModule):
    def __init__(self, gin_params, training_params):
        ...
        self.gnn = GINLSPE(**gin_params)
        self.criterion = nn.L1Loss(reduce='mean')
        self.training_params = training_params

    def training_step(self, batch, batch_idx):
        h, p, edge_index = batch.x, batch[self.training_params['pe_name']], batch.edge_index
        h = h.float()
        p = p.float()
        out = self.gnn(h, p, edge_index, batch.batch)
