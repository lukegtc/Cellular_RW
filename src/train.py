...
from src.topology.pe import (
    AddRandomWalkPE,
    AddCellularRandomWalkPE,
    AppendCCRWPE,
    AppendRWPE,
)
import wandb
...
class LitGNNModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        scheduler = op.lr_scheduler.ReduceLROnPlateau(
            optimizer,
            mode="min",
            factor=self.training_params["lr_decay"],
...
def load_zinc(zinc_folder, use_pe, learnable=False, **pe_params):
    ...
    if use_pe is not None:
        if use_pe == "rw":
            ...
            if not learnable:
                transforms.append(
                    AppendCCRWPE(pe_name=pe_params["attr_name"], use_node_features=True)
                )
        else:
    ...
    data_train = ZINC(
        ...
        transform=transform,
    )  # QM9('datasets/QM9', pre_transform=transform)
    data_val = ZINC(
        zinc_folder,
        subset=True,
        split="val",
...
if __name__ == "__main__":
    ...
    data_train, data_val, data_test = load_zinc(
        args.zinc_folder, args.use_pe, args.learnable_pe, **pe_params
    )
    ...
    training_params = {"lr": 1e-3, "lr_decay": 0.5, "patience": 20, "min_lr": 1e-5}
    ...
    model = LitGNNModel(
        model_name, model_params, training_params, learnable_pe=args.learnable_pe
    )
    wandb.watch(model, log="all")
    if args.use_pe is not None:
        logger = pl.loggers.TensorBoardLogger(
            save_dir=args.log_dir, name=model_name + args.use_pe + args.traverse_type
        )
        ...
        logger = pl.loggers.TensorBoardLogger(
            save_dir=args.log_dir, name=model_name + args.traverse_type
        )
        csv_logger = pl.loggers.CSVLogger(
            save_dir=args.log_dir, name=model_name + args.traverse_type
        )
