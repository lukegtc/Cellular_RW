...
A cellular complex is a generalization of a graph that consists of cells and relations between them. In comparison to graph nodes, cells can be composed into a higher dimensional cell. This flexibility endows cellular complexes with greater expressivity than ordinary graphs, while retaining their structure in a simple way. A cell complex is constructed with a hierarchical gluing procedure. For example, we can view any graph as a 2-dimensional cellular complex with nodes being 0-dimensional cells and edges being 1-dimensional cells composed of two nodes. The procedure can be generalized to higher dimension by considering cycles in graph as a 2-dimensional cells, which lifts the graph to a 3-dimensional CC. The resulting complex incorporates both the original graph and leverages a more comprehensive cellular structure, allowing for a richer representation that can be exploited for a more effective message-passing architecture (Bodnar et al., 2021).

For a cell complex $X$ and a cell $\sigma \in X$, we define four types of cell complex adjacencies:
- The boundary cells are the lower dimensional cells, which belong to $\sigma$;
- The coboundary cells are the upper dimensional cells, which $\sigma$ is a part of;
- The lower adjacent cells are defined as the cells of the same dimension as $\sigma$ that share a lower dimensional cell on their boundary (e.g. edges which share a node);
- The upper adjacent cells are the cells of the same dimension as $\sigma$ that share an upper dimensional cell on their coboundary (e.g. edges which share a cycle).
...
**N-Cellular-RW.** In the initialization process we employ random walks on the upper and lower adjacent cells, defined in the background section. As a baseline, we establish edges for random walks over the lifted graph, we use the upper adjacency index which maps k-dimensional cells to k-dimensional cells. This is similar to ordinary graph adjacency, but in the case of edges, we can jump from one edge to the other if they have a cycle in common. For vertices, this is possible if there is an edge in common, while for cells we do not have this as there is no higher dimensional shared cell. Similarly, we construct lower adjacency index maps that capture the lower adjacency relationships. Finally, we evaluate the combination of both approaches.

**B-Cellular-RW.** This random walk is initialized exclusively using combined boundary and coboundary indices, which effectively force random walks to change dimension of a cell in each step. For example, random walk beginning in a vertex would first need to go to adjacent edge, and in the next step it could choose between stepping into cycle or another node.

**NB-Cellular RW.** Lastly, we combine the two aforementioned random walks. These two methods are combined by allowing the random walk process to traverse both vertically and horizontally over the cellular complex. This means that a walk has the option to either step between cells in the same dimension, effectively creating a shortcut over other lower or higher dimensional complexes or take an extended route that makes use of multiple different dimensions when mapping out the neighboring features. This process has the ability to create a more detailed positional embedding of the surrounding graph strcuture at some computational expense, as this process creates an even more dense adjacencies between cells than the previous two mentioned methods.
