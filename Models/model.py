...
class BasicMPNN(nn.Module):
    def __init__(self, feat_in, pos_in, num_hidden, num_layers):
        super().__init__()
        self.embed = nn.Linear(feat_in + pos_in, num_hidden)
...
class BasicMPNNLayer(nn.Module):
    def __init__(self, num_hidden):
        super().__init__()
        self.message_mlp = nn.Linear(2 * num_hidden, num_hidden)
        self.update_mlp = nn.Linear(2 * num_hidden, num_hidden)

    def forward(self, h, edge_index):
        send, rec = edge_index
        h_send, h_rec = h[send], h[rec]
        messages = self.message_mlp(torch.cat((h_send, h_rec), dim=1))
